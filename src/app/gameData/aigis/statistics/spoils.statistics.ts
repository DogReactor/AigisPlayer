...
import { AL, ALAR } from 'aigis-fuel';
...
export class SpoilsStatistics {
	...
	constructor(private mailBox: AigisStatisticsService) {
		...
		this.buffCalculator.registerBuff(
			60,
			new SpoilsBuff(0, obj => obj === 77 || obj === 133 || obj === 250 || obj === 320)
		);
		...
		this.buffCalculator.registerBuff(
			...
			new SpoilsBuff(
				0,
				obj => this.reference.UnitsList.Rare[obj - 1] === 2 && this.reference.UnitsList.InitClassID[obj - 1] >= 100
			)
		);
	...
	subscribData(gameDataService: AigisGameDataService) {
		...
		gameDataService.subscribe(
			'quest-start',
			(url, response: any, request: any) => {
				// console.log('quest-response', response, request)
				const firstEncounter = request.RT.ATTR.map(e => e === 0);
				Promise.all([
					this.parseSpoils(response, firstEncounter),
					this.buffCalculator.modTeamBuff(request.BL, this.reference)
				])
					.then(
						([dropInfos, sucMsg]) => {
		...
		gameDataService.subscribe(
			file => /QuestNameText(\d+)\.atb/.test(file),
			(url, data: any) => {
